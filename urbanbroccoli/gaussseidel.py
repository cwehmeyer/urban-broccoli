...
def gaussseidel_2d(rho, hx, hy, epsilon, maxiter, maxerr):
    ...
    # check if rho has the right dimension otherwise an error is shown
    if rho.ndim != 2:
    ...
    # initialize variables needed to calculate phi
    nx, ny = rho.shape
    ...
    # loop to calculate phi by iterating the matrix. When
    # the error is smaller than the given maxerr the loop 
    # will be terminated.
    for iteration in range(maxiter):
        error = 0.0
        for x in range(nx):
